...
purpose: Canonical record that code-converge has no hosted runtime stages and defines the boundary with local execution and external CI.
derived_from:
  - ../dna/governance.md
status: active
...
`code-converge` is specified as a local CLI. It has no production, staging, beta, preview, sandbox, hosted control plane, service endpoint, or application database. A Git remote, repository hosting, and CI belong to the target repository and are external systems, not deployment stages of `code-converge`.
...
| Non-local code-converge runtime | N/A | N/A | No hosted runtime exists |
...
- `code-converge` credentials are not provisioned by a hosted environment.
...
- [x] Non-local code-converge environments are explicitly recorded as absent.
