...
- `REQ-04` Fail safely with an actionable diagnostic before review when no candidate is usable or candidates are ambiguous; uniquely resolved stale remote-tracking PR bases are refreshed and revalidated against the provider SHA, while unrefreshable or missing bases fail. Define behavior for detached HEAD, unavailable provider/authentication, forks/remotes ambiguity and an already-merged branch.
...
- `REQ-06` Do not modify the worktree or real Git index to include untracked files. The only implicit remote mutation is a narrowly scoped fetch of a uniquely resolved stale PR base into its existing remote-tracking ref, followed by provider-SHA validation.
...
- `NS-01` Creating, modifying, closing or merging a pull request; changing a remote repository, broad remote synchronization, or fetching a base not uniquely selected from an open PR.
...
- `EC-04` Ambiguous/missing/unrefreshable candidates, detached HEAD and already-merged behavior follow the accepted fail-safe contract before Codex review; a stale unique remote-tracking PR base is refreshed and must match the provider SHA; unavailable `gh` does not break sufficient local resolution.
- `EC-05` Git/hosting discovery and review preserve worktree, real index and hosting objects; the only remote-ref mutation is the verified stale-PR-base refresh.
...
- `NEG-03` No discovery/review path modifies the real index/worktree or creates/updates a PR; it never fetches an explicit, ambiguous, local-only, or non-PR base.
