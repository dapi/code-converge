...
3. `STEP-03` Wire discovery once per workflow run and snapshot refresh before every Codex review; update the Codex adapter to invoke schema-constrained `exec` with a wrapper-prefixed `PATH` and neutral shell-startup values forced through per-invocation Codex shell-policy overrides, disable login-shell startup and remove inherited Git repository/index/config transports and exported shell functions for that review. Keep `GIT_INDEX_FILE` and helper configuration local to snapshot/helper execution via a private wrapper sidecar, reject reviewed-root split-index enabling commands, and set `GIT_EXEC_PATH` only within the wrapper's Git child process (`SOL-03`–`SOL-04`, `INV-01`–`INV-04`, `CHK-01`, `CHK-02`).
...
- `CP-02` After `STEP-03`, assert only snapshot/helper Git mutations carry the temporary `GIT_INDEX_FILE`; every Codex review forces a wrapper-prefixed `PATH` and neutral shell-startup settings through `shell_environment_policy.set`, disables login-shell startup and removes inherited Git transport/config variables and exported shell functions, so profile initialization or caller state cannot bypass or redirect the helper; the wrapper sidecar keeps helper configuration out of restrictive `include_only` environments; every reviewed-root wrapper command uses a disposable index copy and rejects explicit split-index enabling commands, so absolute or nested descendants cannot corrupt the stable snapshot or write shared-index state; and repository creation uses only the target repository index.
- `CP-03` Before publication, map every `REQ-*` to a passing `CHK-*` and an `EVID-*` carrier.
- `STOP-01` If the schema-constrained Codex review invocation does not inherit the wrapper-prefixed `PATH` or cannot inspect the temporary snapshot through scoped Git, stop before implementation alters the public contract; return to `design.md` with observed evidence.
