...
- `CP-02` After `STEP-03`, assert only snapshot/helper Git mutations carry the temporary `GIT_INDEX_FILE`; every Codex review forces a wrapper-prefixed `PATH` through `shell_environment_policy.set`, disables login-shell startup and removes inherited Git transport variables, so profile initialization or caller state cannot bypass or redirect the helper; the wrapper sidecar keeps helper configuration out of restrictive `include_only` environments; every reviewed-root wrapper command uses a disposable index copy so absolute or nested descendant Git cannot corrupt the stable snapshot; and repository creation uses only the target repository index.
