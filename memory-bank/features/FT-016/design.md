...
`CTR-04` also forces neutral `SHELL`, `ZDOTDIR`, `BASH_ENV`, and `ENV` values, and `ReviewTarget.UnsetEnv` removes inherited Git config transports plus exported shell functions. Reviewed-root commands and aliases that explicitly enable split-index fail before Git can create shared-index state. These guards cover non-login startup files and caller-provided command/config injection in addition to the login-shell and repository/index isolation above.
