...
- `SOL-03` Compute the merge-base of `HEAD` and the selected base. Create a private temporary index initialized to that merge-base, then populate it with `git add -A` under `GIT_INDEX_FILE`. Invoke one `codex review --base <selected-base>` with that temporary-index environment so the adapter presents committed, staged, unstaged and untracked worktree content as one Git snapshot without touching the real index/worktree.
...
- `SOL-05` Emit only stable review metadata: `review_scope=branch_and_worktree`, `review_base=<safe ref token>`, `review_merge_base=<full SHA>` and `review_base_source=<stable source>`. Discovery failure is diagnosed on stderr and follows the existing operational-failure path before Codex.
...
- `CTR-02` Discovery source is exactly `explicit`, `open_pr`, `branch_merge_base` or `remote_default`. It selects the first source producing exactly one locally resolvable candidate. A detached HEAD, multiple PR candidates, multiple remote-default candidates, missing current branch, missing selected ref or missing merge-base is an error; no fetch occurs.
- `CTR-03` The temporary index is created outside the repository and is the only mutable Git index used to snapshot the review. `git read-tree <merge-base>` then `git add -A` run with `GIT_INDEX_FILE`; the real index, worktree, refs, remotes and hosting objects are not modified.
...
- `INV-02` The snapshot is merge-base through current worktree state and contains each Git path exactly once; ignored files remain excluded.
...
| `FM-02` | More than one open PR or remote default candidate | Operational failure before Codex; instruct user to set `--review-base`. |
...
- `ALT-02` Alter the real index with `git add -A`. Rejected by `CON-01`/`REQ-06`.
