...
- `REQ-02` Specify and implement human rendering for every current workflow event/result, including local timestamps, stage model/reasoning context, review/fix/CI-recovery budgets, non-zero severity summaries, readable durations, finalization steps and every terminal exit path.
