...
- `SD-07` A findings summary always includes total findings and renders `P0`, `P1`, `P2` in that fixed order. It appends `P3` and `Unknown` only when their counts are non-zero.
- `SD-08` `run_started` is omitted in human mode. Every permanent human line begins with local `HH:MM:SS`; each retryable stage line immediately follows it with `[attempt/max] [model/reasoning-effort]`, with no separator before the message. Review and fix-findings use `cycle/max-cycles`; CI recovery uses `review_phase/max-ci-recoveries`. The overall terminal line has neither prefix because it is not stage-scoped. In an eligible interactive terminal, one startup line announces `Interactive view available: press i to open`; the liveness line replaces the permanent stage-start line; non-TTY keeps the permanent start because it otherwise has no default liveness. While the view has stdin in raw mode, permanent stdout and stderr records use an explicit CRLF terminator so every later record begins at column zero.
...
| `review_completed status=findings` | `[<cycle>/<max-cycles>] Review: <total> findings [P0:<count>; P1:<count>; P2:<count>; P3:<count-if-non-zero>; Unknown:<count-if-non-zero>] (<duration>)` |
...
- `INV-08` While raw mode is active, every permanent human stdout or stderr record explicitly returns to column zero before ending its line.
...
- `FM-09` Raw-mode line feeds preserve the current cursor column and indent later records; permanent raw-mode writes use CRLF and have a regression test.
