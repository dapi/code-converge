...
purpose: "Canonical brief для автоматической публикации проверенных reviewer-артефактов в GitHub Releases."
derived_from:
  - ../../flows/feature.md
...
- `REQ-04` Embed the release version in binaries and expose it through `reviewer --version`; provide a checksum-verifying one-line installer for supported targets.
...
- `SC-04` A release binary prints `reviewer vX.Y.Z` with `reviewer --version`, and the one-line installer downloads the matching archive, verifies its checksum, and installs it under the documented prefix.
...
| `EVID-03` | lint and review result | local runner / reviewer | command output and feature docs | `CHK-03` |
| `EVID-04` | version/installer evidence | local binary and shell checks | `reviewer --version`, `sh -n`, and checksum fixture output | `CHK-01`, `CHK-02`, `CHK-03` |
