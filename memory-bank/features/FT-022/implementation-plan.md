...
| `ER-07` | Tests assert process env but omit Codex-spawned tool policy, shell startup or inherited-variable removal | User shell filters/startup files can hide the scoped wrapper, while caller Git transports or exported functions can redirect the repository/index | Assert the exact invocation-local `shell_environment_policy.set` values for wrapper `PATH` and neutral shell startup, the login-shell override and `ReviewTarget.UnsetEnv` handoff. | Args omit/misquote a scoped value or the runner retains inherited Git/config transports or shell functions. |
