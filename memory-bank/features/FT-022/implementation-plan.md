...
| `internal/repository/review.go` | Resolves selected base/merge base once and refreshes private `GIT_INDEX_FILE` snapshot per review | `REQ-04` target owner | Preserve `ReviewTarget.BaseCommit`, `MergeBase`, `Env`, `Scope` and lifecycle unchanged. |
...
| `PRE-04` | `CON-02`, `CON-05`, `SD-03`, `SD-06`–`SD-08`, `INV-04`, `INV-06` | Existing review target tests remain green and production wiring provides a non-nil `ReviewScope` with selected base, merge base and one `GIT_INDEX_FILE` entry before adapter changes | `STEP-03`–`STEP-05` | yes |
...
| `ER-07` | Tests assert process env but omit Codex-spawned tool policy | User shell-environment filters can hide the private index from the reviewer | Assert the exact invocation-local `shell_environment_policy.set.GIT_INDEX_FILE` override matches process env and prompt. | Args omit or misquote the private-index path. |
