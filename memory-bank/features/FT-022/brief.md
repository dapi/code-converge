...
| `EVID-02` | pass | `TestReviewFakeExecutableBoundary`, scoped-review argument and TOML-encoding tests, repository wrapper isolation tests, inherited-Git/shell-environment removal and startup neutralization, invocation/target assertions and stream-conflict fixtures prove schema/final paths, model/effort, stdin, cwd, wrapper-first `PATH`, absence of Codex-visible `GIT_INDEX_FILE`, merge-base comparison, sole-carrier behavior and cleanup. |
