...
- `REQ-04` Preserve the resolved review base, computed merge base and private `GIT_INDEX_FILE` branch-and-worktree snapshot as inputs available to the reviewing agent, without changing the existing merge-base-to-worktree review scope.
...
- `CON-02` The selected base commit and private `GIT_INDEX_FILE` snapshot remain the canonical review target prepared by `internal/repository`; the Codex adapter must not reconstruct a different target.
...
- `CON-05` Private-index availability must not depend on the operator's unrelated Codex shell-environment whitelist/exclude settings.
...
- `EC-04` The reviewing agent receives the pinned selected base, computed merge base and private `GIT_INDEX_FILE` snapshot prepared for the current review cycle, and compares the private snapshot from the merge base.
...
- `SC-04` The invocation receives the selected base commit and computed merge base in its review instruction, compares from that merge base, and preserves the prepared private-index environment unchanged.
...
| `CHK-02` | `EC-03`, `EC-04`, `SC-03`, `SC-04` | Fake-executable adapter tests for args, stdin, cwd/env, stdout/stderr and output-file behavior | Invocation uses the pinned base/private index and classifies only the named final-response file. | `artifacts/ft-022/verify/chk-02/` |
