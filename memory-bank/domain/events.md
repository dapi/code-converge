...
The structured records and human progress lines emitted to stdout are operational presentation, not a domain-event integration. Their externally visible fields, wording, ordering, liveness and encoding are owned solely by the root [`README.md`](../../README.md); domain and engineering documents may interpret or implement that contract but do not redefine it.
...
- Do not promote a stdout line to a domain event merely because its structured encoding has an `event` field.
