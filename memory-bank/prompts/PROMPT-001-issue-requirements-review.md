...
source_prompt: |
  Перечитай issue или task {{ISSUE_ID}} через доступный tracker и сделай ревью feature по ней
...
variables:
  ...
  - name: ISSUE_COMMAND
    required: false
    description: "Команда или инструмент для чтения issue/task в используемом tracker."
...
| `ISSUE_COMMAND` | no | Как читать issue/task. | `tracker issue view 1234` |
