...
When the review has findings, `code-converge` first requires a clean Git worktree so an automatic checkpoint cannot capture pre-existing user work. It then starts a fresh Codex session with the configured fix-findings prompt followed by the complete classified review report. This gives the stateless remediation session the findings it must address without forwarding the report to workflow stdout. By default, the prompt is:

```text
fix findings
```

The default `fast` profile uses `gpt-5.6-luna` with reasoning effort `medium`. After a successful agent run, `code-converge` checks Git status. If the fix changed the initially clean worktree, it stages the changes and creates one local commit with the stable message `chore: checkpoint review fixes`; it never pushes this checkpoint. A no-change fix attempts no empty commit. A status, staging, commit, branch, or commit-ID failure is operational (exit `2`) and the workflow does not start another review. After a successful checkpoint decision, the workflow returns to **Review**.
...
| `run_completed` | `status=success\|findings_remaining\|operational_failure\|ci_failure`, `exit_code`, and `total_duration_ms`. For `findings_remaining`, also `checkpoint_status=committed_local\|no_changes\|not_attempted`; `committed_local` additionally requires `checkpoint_branch` and `checkpoint_commit`, while `not_attempted` requires `checkpoint_reason`. |
